    ...
    def respond(self, input_text):
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "system", "content": f"Forget all previous conversational context. You are now adopting the following persona: {self.persona}"},
    ...
    def evaluate(self, initial_prompt):
        ...
        for i in range(self.max_interactions): # TODO: 2 - Set loop to iterate up to the maximum number of interactions:
            ...
            response = client.chat.completions.create(
                ...
                {"role": "system", "content": f"Forget all previous context. You are an evaluator with this persona: {self.persona}"},
            ...
            if evaluation.lower().startswith("yes"):
                ...
                response = client.chat.completions.create(
                    ...
                    {"role": "system", "content": "You are a helpful assistant providing correction instructions."},
...
class ActionPlanningAgent:
    ...
    def extract_steps_from_prompt(self, prompt):
        ...
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": f"""You are an action planning agent. 
        ...
        steps_before = response_text.split("\n")
        steps = []
        for line in steps_before:
            clean_line = line.strip()
            # Remove leading bullet points or numbers if they exist
            if clean_line.startswith(("- ", "* ", "• ")):
                clean_line = clean_line[2:]
            elif clean_line and clean_line[0].isdigit() and "." in clean_line[:3]:
                clean_line = clean_line.split(".", 1)[-1]
            
            # Only add if the line is not empty after cleaning
            if clean_line.strip():
