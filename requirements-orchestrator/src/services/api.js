// API integration for the Requirements Orchestrator.
//
// Primary endpoint:
...
//
// If the backend is unreachable (e.g. running standalone), the service falls
// back to a deterministic local mock so the UI stays fully functional.

const GENERATE_ENDPOINT = '/api/agentic-workflow/generate'

/**
 * Orchestrate a BRD into a structured delivery report.
...
/** Ensure the report always exposes the three agent sections in a stable shape. */
...
function mockGenerate(payload) {
  ...
  const report = {
    productManager: {
      ...
      userStories: [
        ...
        },
        {
          ...
          detail: `As a business analyst, I want to track ${subject} items so that I can report progress to stakeholders.`,
        },
        {
          title: 'Receive notifications',
          detail: `As a project manager, I want automated notifications so that I stay informed of ${subject} updates.`,
        },
      ],
      acceptanceCriteria: [
    ...
    programManager: {
      features: [
        { title: 'Intake & Routing', detail: 'Capture BRD inputs and route work items to responsible teams automatically.' },
        { title: 'Progress Dashboard', detail: 'A consolidated view of milestones, dependencies and delivery health.' },
        { title: 'Notification Service', detail: 'Role-based, event-driven alerts across email and in-app channels.' },
      ],
      dependencies: [
    ...
    developmentEngineer: {
      tasks: [
        { title: 'TASK-001 · Define data model', detail: 'Design and migrate the schema for requests, statuses and audit history.' },
        { title: 'TASK-002 · Build intake API', detail: 'Implement the POST endpoint with validation and persistence.' },
        { title: 'TASK-003 · Dashboard UI', detail: 'Create the progress dashboard with live status updates.' },
      ],
      technicalNotes: [
